...
import { Activity, Wifi, Globe, RefreshCw, TrendingUp, TrendingDown, Zap, ChevronLeft, ChevronRight, AlertTriangle, Brain } from "lucide-react" // Import AlertTriangle icon
...
const NetworkMetrics = ({ selectedEnvironment, selectedApp, isDarkMode }) => {
  ...
  const [attackPredictions, setAttackPredictions] = useState([]) // New state for attack predictions
  ...
  useEffect(() => {
    ...
    fetchAttackPredictions() // Fetch attack predictions when filters change
  ...
  // New function to fetch DDoS attack predictions
  ...
  try {
    ...
    console.log("✅ Received attack predictions:", predictions); // <-- ADD THIS
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => {
                fetchNetworkMetrics(currentPage, pageSize);
                fetchAttackPredictions(); // Also refresh predictions
            }}
...
{/* Predicted DDoS Attack Servers - Updated with LLM Label, Scroll, and Modal */}
<div className={`rounded-lg shadow overflow-hidden transition-colors duration-300 ${isDarkMode ? "bg-gray-800" : "bg-white"}`}>
  ...
  {attackPredictions.length > 0 ? (
    <div className="overflow-x-auto max-h-96 overflow-y-auto"> {/* Added scroll container */}
      <table className="min-w-full divide-y divide-gray-200">
        <thead className={`sticky top-0 ${isDarkMode ? "bg-gray-700" : "bg-gray-50"}`}> {/* Sticky header */}
          <tr>
        ...
        <tbody className={`divide-y divide-gray-200 ${isDarkMode ? "bg-gray-800" : "bg-white"}`}>
          {attackPredictions.map((prediction, index) => {
            // Format server ID like "Development-server1"
            ...
            return (
              <tr 
                ...
                onClick={() => setSelectedPrediction(prediction)} // Add click handler
              >
    ...
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-gray-900 mb-2">No DDoS Threats Detected</h3>
      <p className="text-gray-600">All servers are currently operating normally with no predicted DDoS attacks.</p>
    </div>
  )}
</div>
...
{selectedPrediction && (
  <div
    ...
    onClick={() => setSelectedPrediction(null)} // Close on backdrop click
  >
    <div
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-bold">DDoS Attack Prediction Details</h3>
          <span className="px-2 py-1 text-xs font-medium bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-full flex items-center">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className={`mt-2 p-4 rounded-lg border-l-4 border-red-400 ${
